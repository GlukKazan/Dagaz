...
[ -60,  -30,  -10, 20,  20, -10,  -30,   -60, // piecePawn
...
var g_seeValues = [0, 1, 3, 0, 0, 0, 0, 0,
                   0, 1, 3, 0, 0, 0, 0, 0];
...
Dagaz.AI.IsHashMoveValid = function(hashMove) {
    ...
    if (pieceType < piecePawn || pieceType > pieceKnight) return false;
...
Dagaz.AI.ResetGame = function() {
  ...
  var pieceDeltas = [[], g_rookDeltas, g_knightDeltas, [], [], [], []];
  ...
  for (var row = 0; row < 0x80; row += 0x10) {
      for (var col = 0; col < 8; col++) {
           ...
           for (var i = piecePawn; i <= pieceKnight; i++) {
...
Dagaz.AI.GenerateAllMoves = function(moveStack) {
    ...
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
	to = from - 1;  if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from + 1;  if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from + 16; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from - 16; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
        from = Dagaz.AI.g_pieceList[pieceIdx++];
    }
    ...
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
	to = from + 31; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 33; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 14; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 14; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 31; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 33; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 18; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 18; if (Dagaz.AI.g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	from = Dagaz.AI.g_pieceList[pieceIdx++];
    }
}
...
Dagaz.AI.GenerateCaptureMoves = function(moveStack) {
    ...
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
	to = from - 1;  if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from + 1;  if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from + 16; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
	to = from - 16; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion | moveflagPromoteKnight);
        from = Dagaz.AI.g_pieceList[pieceIdx++];
    }
    ...
    from = Dagaz.AI.g_pieceList[pieceIdx++];
    while (from != 0) {
	to = from + 31; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 33; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 14; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 14; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 31; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 33; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from + 18; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	to = from - 18; if (Dagaz.AI.g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
	from = Dagaz.AI.g_pieceList[pieceIdx++];
    }
}
...
Dagaz.AI.See = function(move) {
    ...
    for (var pieceType = piecePawn; pieceType <= pieceKnight; pieceType++) {
        SeeAddKnightAttacks(to, them, themAttacks, pieceType)
    ...
    for (var pieceType = piecePawn; pieceType <= pieceLnight; pieceType++) {
        SeeAddKnightAttacks(to, us, usAttacks, pieceType);
    }
